...
public class ExamsController : ControllerBase
{
    ...
    [Authorize(Roles = UserRoles.ContentManagers)]
    public async Task<ActionResult<ApiResponse<List<QuestionWithAnswerDto>>>> ImportQuestions(Guid id, [FromBody] List<CreateQuestionRequest> requests, CancellationToken cancellationToken)
    {
        var result = await _examService.AddQuestionsBulkAsync(id, requests, Caller, cancellationToken);
